...
const replacements = [
  ...
  ['Offre aller-retour : -15% sur le global', ''],
  ['Aller-retour : retour ajouté au total', ''],
  ['Round trip offer: 15% off the total', ''],
  ['Round trip: return added to total', ''],
  ['Предложение туда-обратно: -15% на общий итог', ''],
  ['Туда-обратно: обратный путь добавлен к итогу', ''],
  ['عرض الذهاب والعودة: خصم 15% على الإجمالي', ''],
  ['الذهاب والعودة: يتم إضافة العودة إلى الإجمالي', ''],
  ['往返优惠：总价减15%', ''],
  ['往返：返程已加入总价', ''],
  ['Hin und zurück: 15% Rabatt auf den Gesamtpreis', ''],
  ['Hin und zurück: Rückfahrt im Gesamtpreis enthalten', ''],
  ['Oferta ida y vuelta: -15% sobre el total', ''],
  ['Ida y vuelta: regreso añadido al total', ''],
  ['Offerta andata e ritorno: -15% sul totale', ''],
  ['Andata e ritorno: ritorno aggiunto al totale', ''],
  ['Oferta ida e volta: -15% no total', ''],
  ['Ida e volta: regresso adicionado ao total', ''],
...
content = content.replace(
  "  const timeLabel = timeOptions.find((x) => x.value === time)?.label || time;\n  const formattedDate = date ? new Intl.DateTimeFormat(l === 'en' ? 'en-US' : l === 'ru' ? 'ru-RU' : l === 'ar' ? 'ar' : l === 'de' ? 'de-DE' : l === 'es' ? 'es-ES' : l === 'it' ? 'it-IT' : l === 'pt' ? 'pt-PT' : l === 'zh' ? 'zh-CN' : 'fr-FR', { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(`${date}T12:00:00`)) : '';",
...
const returnAndFlightBlock = "          {isRoundTrip && <div className=\"grid grid-cols-2 gap-2 md:contents\"><label><span className={labelClass}><CalendarDays size={13}/>{tripModeLabels[l].returnDate}</span><input type=\"date\" value={returnDate} onChange={(e) => setReturnDate(e.target.value)} className={inputClass} /></label><label><span className={labelClass}><Clock size={13}/>{tripModeLabels[l].returnTime}</span><select value={returnTime} onChange={(e) => setReturnTime(e.target.value)} className={inputClass}>{timeOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label></div>}\n" + flightPaxBlock;
...
console.log('[transfer booking] stable round-trip UX patched without discount or offer note');
