...
function normalizeCopyAndVehicles() {
...
function applyAirportDropdowns() {
  ...
  replaceAll(oldPair, newPair);

  const oldPickup = `<label><span className={labelClass}><MapPin size={14}/>{c.pickup}</span><input value={pickup} onChange={(e) => setPickup(e.target.value)} className={inputClass} placeholder={c.pickupPlaceholder}/><AirportQuick/></label>`;
  const newPickup = `<label className="relative"><span className={labelClass}><MapPin size={14}/>{c.pickup}</span><input value={pickup} onFocus={() => setAirportField('pickup')} onChange={(e) => setPickup(e.target.value)} className={inputClass} placeholder={c.pickupPlaceholder}/><AirportDropdown field="pickup"/></label>`;
  replaceAll(oldPickup, newPickup);

  const oldDrop = `<label><span className={labelClass}><MapPin size={14}/>{c.drop}</span><input value={drop} onChange={(e) => setDrop(e.target.value)} className={inputClass} placeholder={c.dropPlaceholder}/></label>`;
  const newDrop = `<label className="relative"><span className={labelClass}><MapPin size={14}/>{c.drop}</span><input value={drop} onFocus={() => setAirportField('drop')} onChange={(e) => setDrop(e.target.value)} className={inputClass} placeholder={c.dropPlaceholder}/><AirportDropdown field="drop"/></label>`;
  replaceAll(oldDrop, newDrop);
...
function layoutReservationFields() {
  ...
  if (content.includes(dateTimeOld) && !content.includes(`grid grid-cols-2 gap-2 md:contents">${dateTimeOld}`)) replaceAll(dateTimeOld, dateTimeNew);
...
if (premiumPaymentUi) {
  const normalized = normalizeCopyAndVehicles();
  ...
  const fieldLayout = layoutReservationFields();
  const polish = modernUiPolish();
  fs.writeFileSync(file, content, 'utf8');
  console.log(`[transfer booking] premium UI patched; normalized ${normalized}, entry currency ${removedCurrency ? 'removed' : 'unchanged'}, Google Places ${disabledGoogle ? 'disabled by default' : 'unchanged'}, dropdowns ${dropdowns}, mobile hero ${hero}, field layout ${fieldLayout}, polish ${polish}`);
  process.exit(0);
}
