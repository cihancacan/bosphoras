...
const replacements = [
  ...
  ['const subtotalBeforeDiscount = effectiveVehiclePrice + effectiveTollPrice + extrasTotal;\n  const roundTripDiscount = isRoundTrip ? Math.round(subtotalBeforeDiscount * 0.15) : 0;\n  const total = subtotalBeforeDiscount - roundTripDiscount + tipValue;', 'const total = vehiclePrice + tollPrice + extrasTotal + tipValue;'],
  ['const total = effectiveVehiclePrice + effectiveTollPrice + extrasTotal + tipValue;', 'const total = vehiclePrice + tollPrice + extrasTotal + tipValue;'],
  ...
  ['body: JSON.stringify({ pickup, dropoff: drop, mode, vehicleId: car.id, hours })', 'body: JSON.stringify({ pickup, dropoff: drop, mode, vehicleId: car.id, hours, roundTrip: mode === \'transfer\' && roundTrip })'],
  ['const oneWayPrice = quote?.total ?? fallbackPrice(car); const price = isRoundTrip ? Math.round(oneWayPrice * 2) : oneWayPrice;', 'const price = quote?.total ?? (isRoundTrip ? Math.round(fallbackPrice(car) * 2) : fallbackPrice(car));'],
  ['{vehicles.map((car) => { const price = quotes[car.id]?.total ?? (isRoundTrip ? Math.round(fallbackPrice(car) * 2) : fallbackPrice(car)); return <button key={car.id}', '{vehicles.map((car) => { const quote = quotes[car.id]; const price = quote?.total ?? (isRoundTrip ? Math.round(fallbackPrice(car) * 2) : fallbackPrice(car)); return <button key={car.id}'],
];
...
content = content.replace("  const vehiclePrice = selectedQuote?.vehiclePrice ?? fallbackPrice(selectedVehicle);\n  const tollPrice = selectedQuote?.tollPrice ?? 0;", "  const vehiclePrice = selectedQuote?.vehiclePrice ?? (isRoundTrip ? fallbackPrice(selectedVehicle) * 2 : fallbackPrice(selectedVehicle));\n  const tollPrice = selectedQuote?.tollPrice ?? 0;");
content = content.replace("  const vehiclePrice = selectedQuote?.vehiclePrice ?? fallbackPrice(selectedVehicle);\n  const tollPrice = selectedQuote?.tollPrice ?? 0;\n  const isRoundTrip = mode === 'transfer' && roundTrip;", "  const vehiclePrice = selectedQuote?.vehiclePrice ?? (isRoundTrip ? fallbackPrice(selectedVehicle) * 2 : fallbackPrice(selectedVehicle));\n  const tollPrice = selectedQuote?.tollPrice ?? 0;");
content = content.replace("  const vehiclePrice = selectedQuote?.vehiclePrice ?? (isRoundTrip ? fallbackPrice(selectedVehicle) * 2 : fallbackPrice(selectedVehicle));\n  const tollPrice = selectedQuote?.tollPrice ?? 0;\n  const isRoundTrip = mode === 'transfer' && roundTrip;", "  const vehiclePrice = selectedQuote?.vehiclePrice ?? (isRoundTrip ? fallbackPrice(selectedVehicle) * 2 : fallbackPrice(selectedVehicle));\n  const tollPrice = selectedQuote?.tollPrice ?? 0;");
content = content.replace("  const extrasTotal = (child ? 30 : 0) + (flowers ? 150 : 0) + (roses ? 400 : 0);\n  const total = effectiveVehiclePrice + effectiveTollPrice + extrasTotal + tipValue;", "  const extrasTotal = (child ? 30 : 0) + (flowers ? 150 : 0) + (roses ? 400 : 0);\n  const total = vehiclePrice + tollPrice + extrasTotal + tipValue;");
...
const newTabs = "<div className=\"mb-4 grid grid-cols-3 rounded-2xl bg-white/45 p-1 backdrop-blur-xl md:bg-gray-100\"><button onClick={() => { setMode('transfer'); setRoundTrip(false); setQuotes({}); }} className={`rounded-xl py-3 text-[10px] font-black uppercase tracking-[0.08em] transition md:text-xs md:tracking-[0.14em] ${mode === 'transfer' && !roundTrip ? 'bg-black text-white shadow-lg' : 'text-gray-700'}`}>{tripModeLabels[l].oneWay}</button><button onClick={() => { setMode('transfer'); setRoundTrip(true); setQuotes({}); }} className={`rounded-xl py-3 text-[10px] font-black uppercase tracking-[0.08em] transition md:text-xs md:tracking-[0.14em] ${mode === 'transfer' && roundTrip ? 'bg-black text-white shadow-lg' : 'text-gray-700'}`}>{tripModeLabels[l].roundTrip}</button><button onClick={() => { setMode('hourly'); setRoundTrip(false); setQuotes({}); }} className={`rounded-xl py-3 text-[10px] font-black uppercase tracking-[0.08em] transition md:text-xs md:tracking-[0.14em] ${mode === 'hourly' ? 'bg-black text-white shadow-lg' : 'text-gray-700'}`}>{tripModeLabels[l].hourly}</button></div>";
...
content = content.replace("{vehicles.map((car) => { const quote = quotes[car.id]; const price = quote?.total ?? fallbackPrice(car); return <button key={car.id}", "{vehicles.map((car) => { const quote = quotes[car.id]; const price = quote?.total ?? (isRoundTrip ? Math.round(fallbackPrice(car) * 2) : fallbackPrice(car)); return <button key={car.id}");
content = content.replace("{vehicles.map((car) => { const quote = quotes[car.id]; const oneWayPrice = quote?.total ?? fallbackPrice(car); const price = isRoundTrip ? Math.round(oneWayPrice * 2) : oneWayPrice; return <button key={car.id}", "{vehicles.map((car) => { const quote = quotes[car.id]; const price = quote?.total ?? (isRoundTrip ? Math.round(fallbackPrice(car) * 2) : fallbackPrice(car)); return <button key={car.id}");
...
console.log('[transfer booking] quote variable restored in vehicle cards');
