...
content = content.replace("{vehicles.map((car) => { const quote = quotes[car.id]; const price = quote?.total ?? fallbackPrice(car); return <button key={car.id}", "{vehicles.map((car) => { const price = quotes[car.id]?.total ?? (isRoundTrip ? Math.round(fallbackPrice(car) * 2) : fallbackPrice(car)); return <button key={car.id}");
...
console.log('[transfer booking] isRoundTrip declared before JSX and quote API receives roundTrip');
