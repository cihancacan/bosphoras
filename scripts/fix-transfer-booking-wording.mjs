...
function addMobileHero() {
  ...
  if (content.includes(gridStart) && !content.includes('data-transfer-mobile-hero')) {
    const mobileHero = '<div data-transfer-mobile-hero="true" className="relative -mx-3 -mt-20 mb-[-34px] h-[420px] overflow-hidden rounded-b-[2rem] md:hidden"><Image src="/images/home.driver.jpg" alt={c.h1} fill priority className="object-cover" sizes="100vw"/><div className="absolute inset-0 bg-gradient-to-b from-black/8 via-black/18 to-black/76"/><div className="absolute bottom-14 left-4 right-4 text-white"><h1 className="text-[2.22rem] font-black leading-[0.94] tracking-[-0.065em]">{c.h1}</h1><div className="mt-5 grid gap-2 text-sm font-extrabold leading-5 text-white"><span>✓ {c.instant}</span><span>✓ {c.refundable}</span><span className="mt-1 inline-flex w-fit items-center gap-2 rounded-full bg-white/95 px-3 py-2 text-[12px] font-black text-slate-950 shadow-lg"><span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-white text-[13px] font-black"><span className="text-[#4285F4]">G</span></span><span className="text-[#fbbc04]">★★★★★</span><span>4,9/5 · 3 358 avis</span></span></div></div></div>';
  ...
  replaceAll('className="mt-5 grid grid-cols-3 rounded-2xl bg-slate-100 p-1"', 'className="mt-5 grid grid-cols-3 gap-2 rounded-none bg-transparent p-0 md:gap-0 md:rounded-2xl md:bg-slate-100 md:p-1"');
  return content !== before;
}
...
function layoutReservationFields() {
  ...
  const returnOld = '{roundTrip && <><label><span className={labelClass}><CalendarDays size={13}/>{c.returnDate}</span><input type="date" min={date} value={returnDate} onChange={(e) => setReturnDate(e.target.value)} className={inputClass}/></label><label><span className={labelClass}><Clock size={13}/>{c.returnTime}</span><select value={returnTime} onChange={(e) => setReturnTime(e.target.value)} className={inputClass}>{timeOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label></>}';
  const returnNew = '{roundTrip && <div className="grid grid-cols-2 gap-2 md:contents"><label><span className={labelClass}><CalendarDays size={13}/>{c.returnDate}</span><input type="date" min={date} value={returnDate} onChange={(e) => setReturnDate(e.target.value)} className={inputClass}/></label><label><span className={labelClass}><Clock size={13}/>{c.returnTime}</span><select value={returnTime} onChange={(e) => setReturnTime(e.target.value)} className={inputClass}>{timeOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label></div>}';
  replaceAll(returnOld, returnNew);
  ...
  const hourlyNew = "{mode === 'hourly' ? <div className=\"grid grid-cols-[74px_minmax(0,1fr)_76px] gap-2 md:contents\"><label><span className={labelClass}><Clock size={14}/>{c.duration}</span><select value={hours} onChange={(e) => setHours(Number(e.target.value))} className={`${inputClass} px-2 text-center`}>{[2,3,4,5,6,8,12].map((h) => <option key={h} value={h}>{h} H</option>)}</select></label><label><span className={labelClass}><Plane size={13}/>{c.flight}</span><input value={flightNumber} onChange={(e) => setFlightNumber(e.target.value)} className={inputClass} placeholder={c.flightPlaceholder}/></label><label><span className={labelClass}><Users size={13}/>{c.pax}</span><input type=\"number\" min={1} max={12} value={pax} onChange={(e) => setPax(Number(e.target.value))} className={`${inputClass} text-center px-2`}/></label></div> : <div className=\"grid grid-cols-[minmax(0,1fr)_84px] gap-2 md:contents\"><label><span className={labelClass}><Plane size={13}/>{c.flight}</span><input value={flightNumber} onChange={(e) => setFlightNumber(e.target.value)} className={inputClass} placeholder={c.flightPlaceholder}/></label><label><span className={labelClass}><Users size={13}/>{c.pax}</span><input type=\"number\" min={1} max={12} value={pax} onChange={(e) => setPax(Number(e.target.value))} className={`${inputClass} text-center px-2`}/></label></div>}";
...
function applyPassengerExtrasToggle() {
  const before = content;

  if (!content.includes('const [showExtras, setShowExtras]')) {
    replaceAll("  const [roses, setRoses] = useState(false);\n  const [tip, setTip] = useState('');", "  const [roses, setRoses] = useState(false);\n  const [showExtras, setShowExtras] = useState(false);\n  const [childQty, setChildQty] = useState(1);\n  const [flowersQty, setFlowersQty] = useState(1);\n  const [rosesQty, setRosesQty] = useState(1);\n  const [tip, setTip] = useState('');");
  }

  replaceAll('const optionsTotal = (child ? extras.child : 0) + (flowers ? extras.flowers : 0) + (roses ? extras.roses : 0) + tipValue;', 'const optionsTotal = (child ? extras.child * childQty : 0) + (flowers ? extras.flowers * flowersQty : 0) + (roses ? extras.roses * rosesQty : 0) + tipValue;');
  ...
  const extrasBlock = '<div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4"><label className="flex cursor-pointer items-center justify-between gap-4"><span><span className="block text-sm font-black text-slate-900">Options supplémentaires</span><span className="block text-xs font-semibold text-slate-500">Activer pour ajouter une note, un siège enfant ou un bouquet</span></span><input type="checkbox" checked={showExtras} onChange={(e) => setShowExtras(e.target.checked)} className="h-5 w-5"/></label>{showExtras && <div className="mt-4 grid gap-3 md:grid-cols-3"><textarea placeholder={c.note} rows={3} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold outline-none transition focus:border-slate-950 focus:ring-4 focus:ring-slate-950/5 md:col-span-3"/><label className="rounded-2xl border border-slate-200 bg-white p-4 text-sm font-bold text-slate-700 transition hover:border-slate-950"><input type="checkbox" checked={child} onChange={(e) => setChild(e.target.checked)}/> <Baby className="my-2 h-5 w-5"/>{c.child}<br/><b>+{formatMoney(extras.child, currency)}</b>{child && <div className="mt-3 flex items-center justify-between gap-2 rounded-xl bg-slate-50 px-3 py-2"><span className="text-xs font-black uppercase text-slate-500">Qté</span><input type="number" min={1} max={6} value={childQty} onChange={(e) => setChildQty(Math.max(1, Number(e.target.value) || 1))} className="w-16 rounded-lg border border-slate-200 bg-white px-2 py-1 text-center text-sm font-black"/></div>}</label><label className="rounded-2xl border border-slate-200 bg-white p-4 text-sm font-bold text-slate-700 transition hover:border-slate-950"><input type="checkbox" checked={flowers} onChange={(e) => setFlowers(e.target.checked)}/> <Flower2 className="my-2 h-5 w-5"/>{c.bouquet}<br/><b>+{formatMoney(extras.flowers, currency)}</b>{flowers && <div className="mt-3 flex items-center justify-between gap-2 rounded-xl bg-slate-50 px-3 py-2"><span className="text-xs font-black uppercase text-slate-500">Qté</span><input type="number" min={1} max={20} value={flowersQty} onChange={(e) => setFlowersQty(Math.max(1, Number(e.target.value) || 1))} className="w-16 rounded-lg border border-slate-200 bg-white px-2 py-1 text-center text-sm font-black"/></div>}</label><label className="rounded-2xl border border-slate-200 bg-white p-4 text-sm font-bold text-slate-700 transition hover:border-slate-950"><input type="checkbox" checked={roses} onChange={(e) => setRoses(e.target.checked)}/> <Flower2 className="my-2 h-5 w-5"/>{c.roses}<br/><b>+{formatMoney(extras.roses, currency)}</b>{roses && <div className="mt-3 flex items-center justify-between gap-2 rounded-xl bg-slate-50 px-3 py-2"><span className="text-xs font-black uppercase text-slate-500">Qté</span><input type="number" min={1} max={20} value={rosesQty} onChange={(e) => setRosesQty(Math.max(1, Number(e.target.value) || 1))} className="w-16 rounded-lg border border-slate-200 bg-white px-2 py-1 text-center text-sm font-black"/></div>}</label></div>}</div><div className="mt-5 rounded-3xl border border-slate-200 bg-slate-50 p-4">';

  content = content.replace(/<div className="mt-6 grid gap-3 md:grid-cols-3"><label className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm font-bold text-slate-700 transition hover:border-slate-950"><input type="checkbox" checked=\{child\}[\s\S]*?<\/div><div className="mt-5 rounded-3xl border border-slate-200 bg-slate-50 p-4">/, extrasBlock);
...
if (premiumPaymentUi) {
  ...
  const extras = applyPassengerExtrasToggle();
  fs.writeFileSync(file, content, 'utf8');
  console.log(`[transfer booking] premium UI patched; entry currency ${removedCurrency ? 'removed' : 'unchanged'}, Google Places ${disabledGoogle ? 'disabled by default' : 'unchanged'}, dropdowns ${dropdowns}, mobile hero ${hero}, fields ${fields}, polish ${polish}, extras ${extras}`);
  process.exit(0);
}
