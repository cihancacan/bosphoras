...
const STEP_GUIDES = 10;

function dedupeGuides(guides: Array<{ href: string; label: string }>) {
...
export function FooterGuideLinks({ locale }: { locale: Locale }) {
  ...
  const registryGuides = allBosphorasSeoPages.map((page) => ({ href: page.slugs[locale], label: page.titles[locale] }));

  const allGuides = dedupeGuides([
    foreignIncomeTaxGuide[locale],
    ...taxClusterGuides[locale],
    ...longTailTaxFooterLinks[locale],
    ...registryGuides,
    ...regularGuides,
  ]);
