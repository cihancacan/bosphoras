...
export const peninsulaCopy: Record<Locale, any> = {
  fr: {
    ...
    gallery: 'Galerie privée', galleryTitle: 'L’adresse principale des membres Bosphoras à Istanbul.', galleryText: 'Ces images ne présentent pas seulement un hôtel. Elles traduisent le cadre dans lequel le membre Bosphoras Premium peut installer une partie de sa vie privée à Istanbul : sport, wellness, rendez-vous, attente discrète, réception et moments de représentation.', finalTitle: 'Demander l’accès membre Bosphoras Premium.', finalText: 'Le Peninsula Istanbul devient l’une des adresses principales de l’expérience Bosphoras Premium : un lieu pour prendre soin de soi, recevoir, travailler et vivre Istanbul avec élégance.', concierge: 'Conciergerie de luxe',
    ...
    galleryImageCaptions: ['Piscine & Bosphore', 'Péninsule historique', 'Hammam privé', 'Fitness premium', 'Espace Spa', 'Abelia Pool Bar', 'Lobby & réception', 'Piscine intérieure'],
  },
  en: {
    ...
    galleryImageCaptions: ['Pool & Bosphorus', 'Historic peninsula', 'Private hammam', 'Premium fitness', 'Spa lounge', 'Abelia Pool Bar', 'Lobby & reception', 'Indoor pool'],
  },
  ru: {
    ...
    galleryImageCaptions: ['Бассейн и Босфор', 'Исторический полуостров', 'Частный хаммам', 'Premium fitness', 'Spa lounge', 'Abelia Pool Bar', 'Лобби и приём', 'Крытый бассейн'],
  },
  ar: {
    ...
    galleryImageCaptions: ['المسبح والبوسفور', 'شبه الجزيرة التاريخية', 'حمام خاص', 'Fitness premium', 'منطقة السبا', 'Abelia Pool Bar', 'اللوبي والاستقبال', 'المسبح الداخلي'],
  },
};
...
export function PeninsulaIstanbulLanding({ locale, currentPath }: { locale: Locale; currentPath: string }) {
  ...
  const rtl = locale === 'ar';
  const galleryImages = [img.pool, img.historic, img.hammam, img.fitness, img.spa, img.abelia, img.lobby, img.interior].map((src, index) => ({ src, label: c.galleryImageCaptions[index] ?? 'The Peninsula Istanbul' }));
  ...
  return (
    <>
      ...
      <main dir={rtl ? 'rtl' : 'ltr'} className="bg-[#f8f1e7] text-[#101827]">
        <section className="relative min-h-[96vh] overflow-hidden bg-[#121826] pt-24 text-[#fffaf0] md:min-h-0 md:pt-32">
          ...
          <div className="absolute inset-0 bg-[linear-gradient(110deg,rgba(0,0,0,0.72)_0%,rgba(0,0,0,0.55)_48%,rgba(0,0,0,0.42)_100%),url('/images/Abelia.Pool.Bar.jpg')] bg-cover bg-center md:hidden" />
          <div className="absolute inset-0 bg-[#20242a]/28" />
          <div className="container-editorial relative z-10 grid min-h-[calc(96vh-6rem)] items-start gap-8 pb-14 pt-6 md:min-h-[80vh] md:pb-12 md:pt-8 lg:grid-cols-[0.95fr_1.05fr] lg:gap-12">
        ...
        <section className="bg-[#fffaf0] px-5 py-20 md:px-8 md:py-28"><div className="mx-auto max-w-[1500px]"><div className="mb-12 grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.24em] text-[#8a6728]">{c.gallery}</p><h2 className="mt-4 font-serif text-4xl tracking-[-0.035em] md:text-6xl">{c.galleryTitle}</h2></div><div className="border border-[#d8c7a1] bg-[#f8f1e7] p-6 shadow-[0_18px_55px_rgba(16,24,39,0.04)]"><p className="text-sm leading-7 text-[#3e4857]">{c.galleryText}</p></div></div><div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">{galleryImages.map((item) => <figure key={item.src} className="group border border-[#d8c7a1] bg-[#fffaf0] p-2 shadow-[0_18px_55px_rgba(16,24,39,0.055)]"><div className="border border-[#d8c7a1]/80 bg-white p-2"><div className="relative aspect-[4/5] overflow-hidden"><img src={item.src} alt={item.label} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /><div className="pointer-events-none absolute inset-3 border border-white/55" /><div className="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,transparent_55%,rgba(16,24,39,0.26)_100%)]" /></div></div><figcaption className="border-x border-b border-[#d8c7a1]/80 bg-white px-4 py-3"><span className="block text-[0.62rem] font-bold uppercase tracking-[0.18em] text-[#8a6728]">{item.label}</span></figcaption></figure>)}</div></div></section>
