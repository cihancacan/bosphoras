...
export function TransferBookingClient({ locale = 'fr' }: { locale?: Locale }) {
  ...
  const vehiclePrice = mode === 'transfer' ? selectedVehicle.rate * (bill / 60) : hourly(selectedVehicle.rate, hours);
  ...
  const findCars = () => { setVehicleId(null); setLoading(true); setTimeout(() => { setLoading(false); setStep(2); }, 5000); };
  ...
  if (step === 1) return <section className="relative min-h-[100svh] overflow-hidden bg-[#eef0f3] px-3 pb-6 pt-20 md:bg-black md:px-8 md:pt-28">
    ...
    <div className="relative z-10 mx-auto max-w-[1180px] md:grid md:min-h-[calc(100svh-8rem)] md:items-center md:gap-6 lg:grid-cols-[0.85fr_1.15fr]">
      ...
      <div className={panelClass}><Stepper /><div className="mb-4 grid grid-cols-2 rounded-2xl bg-white/45 p-1 backdrop-blur-xl md:bg-gray-100"><button onClick={() => setMode('transfer')} className={`rounded-xl py-3 text-xs font-black uppercase tracking-[0.14em] transition ${mode === 'transfer' ? 'bg-black text-white shadow-lg' : 'text-gray-700'}`}>Transfert</button><button onClick={() => setMode('hourly')} className={`rounded-xl py-3 text-xs font-black uppercase tracking-[0.14em] transition ${mode === 'hourly' ? 'bg-black text-white shadow-lg' : 'text-gray-700'}`}>À l’heure</button></div>
        <div className="grid gap-3 md:grid-cols-2"><label><span className={labelClass}><MapPin size={14}/>Départ</span><input ref={pickupRef} list="transfer-hints" value={pickup} onChange={(e) => setPickup(e.target.value)} className={inputClass} placeholder="Adresse ou aéroport" /></label><label><span className={labelClass}><MapPin size={14}/>Arrivée</span><input ref={dropRef} list="transfer-hints" value={drop} onChange={(e) => setDrop(e.target.value)} className={inputClass} placeholder="Adresse ou hôtel" /></label><datalist id="transfer-hints">{hints.map((h) => <option key={h} value={h} />)}</datalist><div className="grid grid-cols-2 gap-2 md:contents"><label><span className={labelClass}><CalendarDays size={13}/>Date</span><input type="date" value={date} onChange={(e) => setDate(e.target.value)} className={inputClass} /></label><label><span className={labelClass}><Clock size={13}/>Heure</span><select value={time} onChange={(e) => setTime(e.target.value)} className={inputClass}>{timeOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label></div><div className="grid grid-cols-[minmax(0,1.35fr)_minmax(70px,0.65fr)] gap-2 md:contents"><label><span className={labelClass}><Plane size={13}/>N° de vol</span><input value={flightNumber} onChange={(e) => setFlightNumber(e.target.value)} className={inputClass} placeholder="Pour être accueilli" /></label><label><span className={labelClass}><Users size={13}/>Passagers</span><input type="number" min={1} max={12} value={pax} onChange={(e) => setPax(Number(e.target.value))} className={`${inputClass} text-center`} /></label></div>{mode === 'hourly' && <label className="md:col-span-2"><span className={labelClass}><Clock size={14}/>Durée</span><select value={hours} onChange={(e) => setHours(Number(e.target.value))} className={inputClass}>{[2,3,4,5,6,8,12].map((h) => <option key={h} value={h}>{h} heures</option>)}</select></label>}</div>
