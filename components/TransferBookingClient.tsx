...
export function TransferBookingClient({ locale: _locale = 'fr' }: { locale?: Locale }) {
  ...
  return (
    <div className="mx-auto max-w-[1180px] px-5 py-10 md:px-8 md:py-14">
      <div className="mb-8 grid grid-cols-3 overflow-hidden border border-[#d8c7a1] bg-white">
        {stepLabels.map((label, index) => {
          const active = step === index + 1;
          const done = step > index + 1;
          return <div key={label} className={`flex items-center justify-center gap-2 border-r border-[#d8c7a1] px-3 py-4 text-center text-[0.68rem] font-bold uppercase tracking-[0.12em] last:border-r-0 ${active ? 'bg-[#121826] text-[#fffaf0]' : done ? 'bg-[#fffaf0] text-[#8a6728]' : 'bg-white text-[#5c6676]'}`}>{done && <Check size={14} />}{index + 1}. {label}</div>;
        })}
      </div>
        ...
        <section>
          ...
          <div className="grid gap-5">
            {vehicles.map((vehicle) => {
              ...
              return <button key={vehicle.id} onClick={() => setSelectedVehicleId(vehicle.id)} className={`grid w-full gap-5 border p-4 text-left transition md:grid-cols-[260px_1fr_auto] md:items-center ${isSelected ? 'border-[#8a6728] bg-[#fffaf0] shadow-[0_18px_65px_rgba(16,24,39,0.12)]' : 'border-[#d8c7a1] bg-white hover:border-[#8a6728]'}`}><div className="relative h-44 overflow-hidden bg-[#f8f1e7] md:h-36"><Image src={vehicle.image} alt={vehicle.name} fill className="object-cover" sizes="(min-width: 768px) 260px, 100vw" /></div><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#8a6728]">{vehicle.label}</p><h3 className="mt-2 font-serif text-3xl leading-tight text-[#121826]">{vehicle.name}</h3><p className="mt-3 text-sm leading-7 text-[#5c6676]">{vehicle.description}</p><div className="mt-4 flex flex-wrap gap-3 text-xs font-semibold uppercase tracking-[0.10em] text-[#3e4857]"><span className="inline-flex items-center gap-2"><Users size={15} />{vehicle.passengers}</span><span className="inline-flex items-center gap-2"><Luggage size={15} />{vehicle.luggage}</span><span className="inline-flex items-center gap-2"><Plane size={15} />Suivi de vol inclus</span><span className="inline-flex items-center gap-2"><Check size={15} />Accueil inclus</span></div></div><div className="min-w-[190px] border-t border-[#d8c7a1] pt-4 md:border-l md:border-t-0 md:pl-6 md:pt-0"><p className="text-xs uppercase tracking-[0.16em] text-[#5c6676]">Prix</p><p className="mt-2 font-serif text-4xl text-[#121826]">{formatEuro(basePrice)}</p><p className="mt-2 text-xs leading-5 text-[#5c6676]">{formatEuro(vehicle.hourlyRate)} / h</p><span className={`mt-4 inline-flex items-center justify-center px-5 py-3 text-xs font-bold uppercase tracking-[0.14em] ${isSelected ? 'bg-[#121826] text-[#fffaf0]' : 'border border-[#8a6728] text-[#8a6728]'}`}>{isSelected ? 'Sélectionné' : 'Choisir'}</span></div></button>;
            })}
          </div>
          <div className="mt-8 flex flex-col justify-between gap-3 sm:flex-row"><button onClick={() => setStep(1)} className="inline-flex items-center justify-center gap-3 border border-[#8a6728] px-7 py-4 text-xs font-bold uppercase tracking-[0.14em] text-[#8a6728]"><ArrowLeft size={15} />Retour</button><button onClick={() => setStep(3)} className="inline-flex items-center justify-center gap-3 bg-[#121826] px-8 py-4 text-xs font-bold uppercase tracking-[0.16em] text-[#fffaf0]">Continuer<ArrowRight size={16} /></button></div>
        </section>
      )}

      {step === 3 && (
        <section className="grid gap-8 lg:grid-cols-[1fr_380px]">
          <div className="border border-[#d8c7a1] bg-white p-6 md:p-8"><h2 className="font-serif text-4xl tracking-[-0.035em] text-[#121826]">Informations passager</h2><div className="mt-7 grid gap-3 sm:grid-cols-2"><button onClick={() => setBookingFor('self')} className={`border px-5 py-4 text-left text-sm font-bold uppercase tracking-[0.12em] ${bookingFor === 'self' ? 'border-[#8a6728] bg-[#f8f1e7] text-[#121826]' : 'border-[#d8c7a1] text-[#5c6676]'}`}>Pour moi-même</button><button onClick={() => setBookingFor('other')} className={`border px-5 py-4 text-left text-sm font-bold uppercase tracking-[0.12em] ${bookingFor === 'other' ? 'border-[#8a6728] bg-[#f8f1e7] text-[#121826]' : 'border-[#d8c7a1] text-[#5c6676]'}`}>Pour une autre personne</button></div><div className="mt-6 grid gap-4 md:grid-cols-2">{['Nom', 'Prénom', 'Téléphone WhatsApp', 'Adresse e-mail'].map((label) => <label key={label} className="block"><span className="mb-2 block text-xs font-bold uppercase tracking-[0.16em] text-[#8a6728]">{label}</span><input className="w-full border border-[#d8c7a1] bg-[#fffaf0] px-4 py-4 text-sm outline-none transition focus:border-[#8a6728]" /></label>)}<label className="block md:col-span-2"><span className="mb-2 block text-xs font-bold uppercase tracking-[0.16em] text-[#8a6728]">Note / souhait particulier</span><textarea rows={4} className="w-full border border-[#d8c7a1] bg-[#fffaf0] px-4 py-4 text-sm outline-none transition focus:border-[#8a6728]" /></label><label className="block md:col-span-2"><span className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-[#8a6728]"><Plane size={15} />Numéro de vol optionnel</span><input className="w-full border border-[#d8c7a1] bg-[#fffaf0] px-4 py-4 text-sm outline-none transition focus:border-[#8a6728]" placeholder="Exemple : TK1828" /></label></div><div className="mt-8 border-t border-[#d8c7a1] pt-8"><p className="text-xs font-bold uppercase tracking-[0.26em] text-[#8a6728]">Services additionnels</p><div className="mt-5 grid gap-4 md:grid-cols-3"><label className="flex cursor-pointer gap-3 border border-[#d8c7a1] bg-[#fffaf0] p-4"><input type="checkbox" checked={childSeat} onChange={(event) => setChildSeat(event.target.checked)} /><span><Baby className="mb-3 h-5 w-5 text-[#8a6728]" /><strong>Siège enfant</strong><br /><span className="text-sm text-[#5c6676]">+30 €</span></span></label><label className="flex cursor-pointer gap-3 border border-[#d8c7a1] bg-[#fffaf0] p-4"><input type="checkbox" checked={flowerSelection} onChange={(event) => setFlowerSelection(event.target.checked)} /><span><Flower2 className="mb-3 h-5 w-5 text-[#8a6728]" /><strong>Bouquet sélection</strong><br /><span className="text-sm text-[#5c6676]">+150 €</span></span></label><label className="flex cursor-pointer gap-3 border border-[#d8c7a1] bg-[#fffaf0] p-4"><input type="checkbox" checked={redRoses} onChange={(event) => setRedRoses(event.target.checked)} /><span><Flower2 className="mb-3 h-5 w-5 text-[#8a6728]" /><strong>33 roses rouges</strong><br /><span className="text-sm text-[#5c6676]">+400 €</span></span></label></div><label className="mt-5 block"><span className="mb-2 block text-xs font-bold uppercase tracking-[0.16em] text-[#8a6728]">Pourboire — montant libre</span><input inputMode="decimal" value={tip} onChange={(event) => setTip(event.target.value)} className="w-full border border-[#d8c7a1] bg-[#fffaf0] px-4 py-4 text-sm outline-none transition focus:border-[#8a6728]" placeholder="Exemple : 20" /></label></div><div className="mt-8"><button onClick={() => setStep(2)} className="inline-flex items-center justify-center gap-3 border border-[#8a6728] px-7 py-4 text-xs font-bold uppercase tracking-[0.14em] text-[#8a6728]"><ArrowLeft size={15} />Retour</button></div></div>
          <aside className="sticky top-28 self-start border border-[#24334a] bg-[#121826] p-6 text-[#fffaf0] md:p-8"><p className="text-xs font-bold uppercase tracking-[0.24em] text-[#d2a863]">Résumé</p><h3 className="mt-4 font-serif text-3xl leading-tight">Paiement sécurisé</h3><div className="mt-6 space-y-4 text-sm leading-7 text-[#d8cfbf]"><p><strong className="text-white">Service :</strong> {mode === 'transfer' ? 'Transfert privé' : `Chauffeur · ${hours}h`}</p><p><strong className="text-white">Trajet :</strong> {pickup} → {dropoff}</p><p><strong className="text-white">Véhicule :</strong> {selectedVehicle.name}</p><p><strong className="text-white">Passagers :</strong> {passengers}</p></div><div className="my-6 h-px bg-[#d2a863]/30" /><div className="space-y-3 text-sm text-[#d8cfbf]"><div className="flex justify-between"><span>Véhicule</span><span>{formatEuro(vehiclePrice)}</span></div>{childSeat && <div className="flex justify-between"><span>Siège enfant</span><span>{formatEuro(30)}</span></div>}{flowerSelection && <div className="flex justify-between"><span>Bouquet sélection</span><span>{formatEuro(150)}</span></div>}{redRoses && <div className="flex justify-between"><span>33 roses rouges</span><span>{formatEuro(400)}</span></div>}{tipAmount > 0 && <div className="flex justify-between"><span>Pourboire</span><span>{formatEuro(tipAmount)}</span></div>}</div><div className="mt-6 flex items-end justify-between border-t border-[#d2a863]/30 pt-6"><span className="text-xs font-bold uppercase tracking-[0.18em] text-[#d2a863]">Total</span><span className="font-serif text-5xl">{formatEuro(total)}</span></div><button className="mt-7 flex w-full items-center justify-center gap-3 bg-[#d2a863] px-6 py-4 text-xs font-bold uppercase tracking-[0.14em] text-[#101827] transition hover:bg-[#e0bc78]">Payer et confirmer<ArrowRight size={16} /></button><p className="mt-4 text-xs leading-6 text-[#d8cfbf]">Paiement uniquement par carte. Confirmation immédiate après paiement.</p></aside>
        </section>
      )}
    </div>
  );
