      ...
      <div className="relative z-10 mx-auto grid min-h-[calc(100svh-8rem)] max-w-[1180px] items-end gap-6 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        ...
        <div className="rounded-[28px] bg-white/95 p-4 shadow-[0_30px_110px_rgba(0,0,0,0.35)] backdrop-blur-xl md:p-6">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <label><span className="mb-1 flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.14em] text-gray-500"><Clock size={14}/>Heure</span><select value={time} onChange={(e) => setTime(e.target.value)} className="h-14 w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 text-sm font-semibold outline-none focus:border-black">{timeOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
            ...
            <label className={mode === 'hourly' ? '' : 'md:col-span-2'}><span className="mb-1 flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.14em] text-gray-500"><Plane size={14}/>Numéro de vol pour être accueilli</span><input value={flightNumber} onChange={(e) => setFlightNumber(e.target.value)} className="h-14 w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 text-sm font-semibold outline-none focus:border-black" placeholder="Optionnel — ex : TK1828" /></label>
          </div>
          {loading ? <div className="mt-5 rounded-3xl border border-gray-200 bg-gray-50 p-6 text-center"><div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-white shadow-inner"><div className="absolute h-16 w-16 animate-spin rounded-full border-4 border-gray-200 border-t-black"/><Timer className="relative h-7 w-7"/></div><p className="text-lg font-black tracking-[-0.03em]">Recherche des véhicules disponibles</p><p className="mt-1 text-sm text-gray-500">Vérification du trajet, de l’horaire, du vol et des passagers...</p></div> : <div className="mt-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><p className="text-xs font-bold text-gray-500">Minimum 2h · horaires par 10 min · suivi de vol si renseigné</p><button onClick={findCars} className="inline-flex items-center justify-center gap-3 rounded-2xl bg-black px-7 py-4 text-xs font-black uppercase tracking-[0.14em] text-white shadow-xl transition hover:-translate-y-0.5">Voir les véhicules<ArrowRight size={16}/></button></div>}
        </div>
      </div>
    </section>
  );
