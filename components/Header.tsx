  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-50 border-b border-[hsl(220,35%,15%)] bg-[hsl(220,45%,8%)] shadow-[0_18px_45px_rgba(0,0,0,0.16)]">
      ...
      <div className="container-editorial flex h-[4.9rem] items-center justify-between md:h-[5.4rem]">
        ...
        <div className="hidden items-center gap-4 lg:flex"><div className="relative"><button onClick={() => setLangOpen(!langOpen)} className="flex items-center gap-2 text-[0.64rem] font-semibold uppercase tracking-[0.18em] text-[hsl(220,10%,55%)] transition-colors duration-200 hover:text-[hsl(42,65%,52%)]" aria-expanded={langOpen} aria-haspopup="listbox">{localeNames[locale]}<span className="text-[hsl(42,65%,45%)]">▾</span></button>{langOpen && <div className="absolute right-0 top-full z-50 mt-4 min-w-[150px] border border-[hsl(42,65%,52%)]/25 bg-[hsl(220,45%,8%)] shadow-[0_28px_80px_rgba(0,0,0,0.32)]" role="listbox">{locales.map((loc) => <Link key={loc} href={getLocaleHref(loc)} onClick={() => setLangOpen(false)} className={`block px-4 py-3 text-xs uppercase tracking-[0.12em] transition-colors duration-150 hover:bg-white/5 ${loc === locale ? 'font-semibold text-[hsl(42,65%,52%)]' : 'text-[hsl(220,10%,55%)]'}`} role="option" aria-selected={loc === locale} lang={loc}>{localeNames[loc]}</Link>)}</div>}</div><Link href={linkTo('private-assessment')} className="border border-[hsl(42,65%,52%)] bg-[hsl(42,65%,52%)] px-6 py-3 text-[0.64rem] font-bold uppercase tracking-[0.16em] text-[hsl(220,45%,8%)] transition-colors duration-300 hover:bg-[hsl(42,65%,60%)]">{t(locale, 'nav.assessment')}</Link></div>
        <button className="p-2 text-[hsl(45,30%,96%)] lg:hidden" onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? 'Fermer le menu' : 'Ouvrir le menu'} aria-expanded={menuOpen}>{menuOpen ? <X size={22} /> : <Menu size={22} />}</button>
      </div>
