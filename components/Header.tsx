...
const specialLocalizedPaths: Array<Record<Locale, string>> = [
  ...
  { fr: '/agro-industrie-turquie-investissement-fiscalite', en: '/en/agribusiness-turkey-investment-tax', ru: '/ru/agropromyshlennost-turtsiya-investitsii-nalogi', ar: '/ar/agribusiness-turkey-investment-tax' },
  { fr: '/startup-turquie-avantages-fiscaux-investisseurs', en: '/en/startup-turkey-tax-benefits-investors', ru: '/ru/startup-turtsiya-nalogovye-lgoty-investory', ar: '/ar/startup-turkey-tax-benefits-investors' },
  { fr: '/finance-fintech-turquie-istanbul-finance-center', en: '/en/finance-fintech-turkey-istanbul-finance-center', ru: '/ru/finansy-fintech-turtsiya-istanbul-finance-center', ar: '/ar/finance-fintech-turkey-istanbul-finance-center' },
  { fr: '/import-export-turquie-transit-trade', en: '/en/import-export-turkey-transit-trade', ru: '/ru/import-export-turtsiya-tranzitnaya-torgovlya', ar: '/ar/import-export-turkey-transit-trade' },
  { fr: '/holding-siege-regional-turquie', en: '/en/holding-regional-headquarters-turkey', ru: '/ru/holding-regionalnyy-ofis-turtsiya', ar: '/ar/holding-regional-headquarters-turkey' },
  { fr: '/residence-fiscale-turquie-immobilier-investisseurs', en: '/en/turkey-tax-residence-property-investors', ru: '/ru/nalogovaya-rezidentsiya-turtsiya-nedvizhimost-investory', ar: '/ar/turkey-tax-residence-property-investors' },
  { fr: '/turquie-vs-portugal-residence-fiscale-investisseurs', en: '/en/turkey-vs-portugal-tax-residence-investors', ru: '/ru/turtsiya-vs-portugaliya-nalogovaya-rezidentsiya', ar: '/ar/turkey-vs-portugal-tax-residence-investors' },
  { fr: '/turquie-vs-monaco-family-office-residence', en: '/en/turkey-vs-monaco-family-office-residence', ru: '/ru/turtsiya-vs-monako-family-office-rezidentsiya', ar: '/ar/turkey-vs-monaco-family-office-residence' },
  { fr: '/turquie-singapour-hong-kong-transit-trade', en: '/en/turkey-singapore-hong-kong-transit-trade', ru: '/ru/turtsiya-singapur-gonkong-tranzitnaya-torgovlya', ar: '/ar/turkey-singapore-hong-kong-transit-trade' },
...
export function Header({ locale, currentPath = '/' }: HeaderProps) {
  ...
  const groups: MenuGroup[] = [
    { label: labels.services, href: linkTo('services'), items: [
      { label: locale === 'fr' ? 'Installation en Turquie' : locale === 'en' ? 'Relocate to Turkey' : locale === 'ru' ? 'Переезд в Турцию' : 'الانتقال إلى تركيا', href: linkTo('relocate'), description: locale === 'fr' ? 'Résidence, famille, ville, démarches.' : locale === 'en' ? 'Residence, family, city, formalities.' : locale === 'ru' ? 'Резиденция, семья, город, процедуры.' : 'الإقامة، العائلة، المدينة والإجراءات.' },
      { label: locale === 'fr' ? 'Conseil Juridique & Fiscal' : locale === 'en' ? 'Legal & Tax Advisory' : locale === 'ru' ? 'Право и налоги' : 'القانون والضرائب', href: linkTo('legal-tax'), description: locale === 'fr' ? 'Avocats, fiscalistes, conformité.' : locale === 'en' ? 'Lawyers, tax advisors, compliance.' : locale === 'ru' ? 'Юристы, налоги, compliance.' : 'محامون، ضرائب وامتثال.' },
    ...
    { label: labels.privateAccess, href: linkTo('private-club'), items: [{ label: locale === 'fr' ? 'Cercle privé' : locale === 'en' ? 'Private Access Club' : locale === 'ru' ? 'Частный клуб' : 'النادي الخاص', href: linkTo('private-club') }, { label: locale === 'fr' ? 'Demande d’adhésion' : locale === 'en' ? 'Membership application' : locale === 'ru' ? 'Заявка на членство' : 'طلب عضوية', href: linkTo('membership-application') }, { label: locale === 'fr' ? 'Diagnostic privé' : locale === 'en' ? 'Private Assessment' : locale === 'ru' ? 'Частная консультация' : 'تقييم خاص', href: linkTo('private-assessment') }] },
    ...
    <>
      <header className="fixed left-0 right-0 top-0 z-50 border-b border-[hsl(220,35%,15%)] bg-[hsl(220,45%,8%)] shadow-[0_18px_45px_rgba(0,0,0,0.16)]">
        ...
        <div className="container-editorial flex h-[4.9rem] items-center justify-between md:h-[5.4rem]">
          <Link href={getLocalePath(locale, '/')} className="group flex flex-col leading-none" aria-label="Bosphoras — Accueil"><span className="font-serif text-[1.22rem] font-normal uppercase tracking-[0.22em] text-[hsl(45,30%,96%)] transition-colors duration-300 group-hover:text-[hsl(42,65%,52%)] md:text-[1.35rem]">Bosphoras</span><span className="mt-1 text-[0.54rem] uppercase tracking-[0.30em] text-[hsl(42,65%,45%)]">{locale === 'fr' && 'Bureau Privé · Turquie'}{locale === 'en' && 'Private Office · Turkey'}{locale === 'ru' && 'Частный Офис · Турция'}{locale === 'ar' && 'مكتب خاص · تركيا'}</span></Link>
          <nav className="hidden items-center gap-7 lg:flex" aria-label="Navigation principale">{groups.map((group) => <div key={group.label} className="group relative py-7"><Link href={group.href} className="inline-flex items-center gap-2 text-[0.64rem] font-semibold uppercase tracking-[0.18em] text-[hsl(220,10%,55%)] transition-colors duration-200 hover:text-[hsl(42,65%,52%)]">{group.label}<span className="text-[hsl(42,65%,45%)]">▾</span></Link><div className="invisible absolute left-0 top-full z-50 w-[360px] translate-y-2 border border-[hsl(42,65%,52%)]/25 bg-[hsl(220,45%,8%)] p-3 opacity-0 shadow-[0_28px_80px_rgba(0,0,0,0.32)] transition-all duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100">{group.items.map((item) => <Link key={item.href} href={item.href} className="block border-b border-white/5 px-4 py-3 last:border-b-0 hover:bg-white/[0.04]"><span className="block text-xs font-semibold uppercase tracking-[0.14em] text-[hsl(45,30%,96%)]">{item.label}</span>{item.description && <span className="mt-1 block text-xs leading-5 text-[hsl(220,10%,55%)]">{item.description}</span>}</Link>)}</div></div>)}</nav>
          <div className="hidden items-center gap-3 lg:flex">
            <button onClick={() => setSearchOpen(true)} className="inline-flex items-center gap-2 border border-white/10 px-4 py-3 text-[0.64rem] font-bold uppercase tracking-[0.14em] text-[hsl(45,30%,96%)] transition-colors duration-200 hover:border-[hsl(42,65%,52%)] hover:text-[hsl(42,65%,52%)]" aria-label={searchLabel}><Search size={15} />{searchLabel}</button>
            <div className="relative"><button onClick={() => setLangOpen(!langOpen)} className="flex h-10 items-center gap-2 border border-white/10 px-3 text-[0.64rem] font-semibold uppercase tracking-[0.14em] text-[hsl(220,10%,65%)] transition-colors duration-200 hover:border-[hsl(42,65%,52%)] hover:text-[hsl(42,65%,52%)]" aria-expanded={langOpen} aria-haspopup="listbox"><Globe2 size={15} /><span>{locale.toUpperCase()}</span><span className="text-[hsl(42,65%,45%)]">▾</span></button>{langOpen && <div className="absolute right-0 top-full z-50 mt-4 min-w-[150px] border border-[hsl(42,65%,52%)]/25 bg-[hsl(220,45%,8%)] shadow-[0_28px_80px_rgba(0,0,0,0.32)]" role="listbox">{locales.map((loc) => <Link key={loc} href={getLocaleHref(loc)} onClick={() => setLangOpen(false)} className={`block px-4 py-3 text-xs uppercase tracking-[0.12em] transition-colors duration-150 hover:bg-white/5 ${loc === locale ? 'font-semibold text-[hsl(42,65%,52%)]' : 'text-[hsl(220,10%,55%)]'}`} role="option" aria-selected={loc === locale} lang={loc}>{localeNames[loc]}</Link>)}</div>}</div>
            <Link href={linkTo('private-assessment')} className="border border-[hsl(42,65%,52%)] bg-[hsl(42,65%,52%)] px-5 py-3 text-[0.64rem] font-bold uppercase tracking-[0.14em] text-[hsl(220,45%,8%)] transition-colors duration-300 hover:bg-[hsl(42,65%,60%)]">{t(locale, 'nav.assessment')}</Link>
          </div>
          <div className="flex items-center gap-2 lg:hidden">
            <button className="p-2 text-[hsl(45,30%,96%)]" onClick={() => setSearchOpen(true)} aria-label={searchLabel}><Search size={21} /></button>
            <button className="p-2 text-[hsl(45,30%,96%)]" onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? 'Fermer le menu' : 'Ouvrir le menu'} aria-expanded={menuOpen}>{menuOpen ? <X size={22} /> : <Menu size={22} />}</button>
          </div>
        </div>
