...
export function Header({ locale, currentPath = '/' }: HeaderProps) {
  ...
  const groups: MenuGroup[] = [
    { label: labels.services, href: linkTo('services'), items: [
      ...
      { label: locale === 'fr' ? 'Transferts & Chauffeur' : locale === 'en' ? 'Transfers & Chauffeur' : locale === 'ru' ? 'Трансферы и водитель' : 'النقل والسائق الخاص', href: mobilityLink, description: locale === 'fr' ? 'Transfert aeroport, chauffeur a l heure, vehicules premium.' : locale === 'en' ? 'Airport transfer, hourly chauffeur, premium vehicles.' : locale === 'ru' ? 'Трансфер из аэропорта, водитель по часам, премиальные автомобили.' : 'نقل المطار، سائق بالساعة، سيارات فاخرة.' },
    ...
    { label: labels.information, href: linkTo('about'), items: [
      { label: locale === 'fr' ? 'A propos' : locale === 'en' ? 'About' : locale === 'ru' ? 'О нас' : 'من نحن', href: linkTo('about') },
      { label: locale === 'fr' ? 'Conformite & confiance' : locale === 'en' ? 'Compliance & Trust' : locale === 'ru' ? 'Комплаенс и доверие' : 'الامتثال والثقة', href: linkTo('compliance') },
      { label: locale === 'fr' ? 'Partenaires & reseau' : locale === 'en' ? 'Partners & Network' : locale === 'ru' ? 'Партнеры и сеть' : 'الشركاء والشبكة', href: linkTo('partners') },
      { label: locale === 'fr' ? 'Questions frequentes' : locale === 'en' ? 'FAQ' : locale === 'ru' ? 'Вопросы и ответы' : 'أسئلة شائعة', href: linkTo('faq') },
    ]},
  ];
      ...
      <header className="fixed left-0 right-0 top-0 z-50 border-b border-[hsl(220,35%,15%)] bg-[hsl(220,45%,8%)] shadow-[0_18px_45px_rgba(0,0,0,0.16)]">
        ...
        <div className="container-editorial flex h-[4.9rem] items-center justify-between md:h-[5.4rem]">
          ...
          <div className="hidden items-center gap-3 lg:flex">
            ...
            <Link href={linkTo('private-assessment')} data-cta-id="header_assessment" className="border border-[hsl(42,65%,52%)] bg-[hsl(42,65%,52%)] px-5 py-3 text-[0.64rem] font-bold uppercase tracking-[0.14em] text-[hsl(220,45%,8%)] transition-colors duration-300 hover:bg-[hsl(42,65%,60%)]">{t(locale, 'nav.assessment')}</Link>
          </div>
