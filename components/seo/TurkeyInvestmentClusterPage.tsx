...
function pageIntro(key: TaxClusterKey, locale: Locale) {
  if (key !== 'taxStrategy') return fallbackIntro[locale];
...
function taxStrategyBullets(locale: Locale): Array<[string,string]> {
  const data: Record<Locale, Array<[string,string]>> = {
    fr: [
      ['Un cadre fiscal officiellement publié', 'La loi n°7582 introduit l’article 20/D sur certains revenus étrangers, mais aussi des mesures sur héritage, centres de services, transit trade, production et Istanbul Finance Center.'],
      ['Une stratégie par profil, pas une promesse automatique', 'Un investisseur étranger doit vérifier résidence fiscale, source des revenus, trois années précédentes, conventions fiscales, banque, substance réelle et pays d’origine.'],
      ['Des cas concrets pour investisseurs', 'Résidence privée, family office, holding opérationnelle, centre régional, transit trade, industrie exportatrice ou base Istanbul Finance Center peuvent être étudiés séparément.'],
      ['Un dossier à documenter', 'La bonne approche consiste à préparer preuves de résidence, origine des fonds, structure de revenus, sociétés, actifs, famille, succession et calendrier d’installation.'],
    ],
    en: [
      ['An officially published tax framework', 'Law No. 7582 introduces Article 20/D on certain foreign income, together with measures on inheritance, service centers, transit trade, manufacturing and Istanbul Finance Center.'],
      ['A profile-based strategy, not an automatic promise', 'A foreign investor must verify tax residence, income source, previous three years, tax treaties, banking, real substance and country of origin.'],
      ['Concrete investor cases', 'Private residence, family office, operating holding, regional center, transit trade, export manufacturing or an Istanbul Finance Center base can be reviewed separately.'],
      ['A file to document', 'The correct approach is to prepare residence evidence, source of funds, income structure, companies, assets, family, succession and relocation timeline.'],
    ],
    ru: [
      ['Официально опубликованная налоговая рамка', 'Закон №7582 вводит статью 20/D по отдельным иностранным доходам, а также меры по наследованию, сервисным центрам, transit trade, производству и Istanbul Finance Center.'],
      ['Стратегия по профилю, не автоматическое обещание', 'Иностранный инвестор должен проверить налоговую резиденцию, источник доходов, три предыдущих года, налоговые соглашения, банк, substance и страну происхождения.'],
      ['Практические сценарии для инвесторов', 'Частная резиденция, family office, операционный холдинг, региональный центр, transit trade, экспортное производство или база Istanbul Finance Center анализируются отдельно.'],
      ['Досье нужно документировать', 'Правильный подход — подготовить доказательства резиденции, источник средств, структуру доходов, компании, активы, семью, наследование и график переезда.'],
    ],
    ar: [
      ['إطار ضريبي منشور رسمياً', 'القانون رقم 7582 يضيف المادة 20/D حول بعض الدخل الأجنبي، مع تدابير حول الميراث ومراكز الخدمات والتجارة العابرة والصناعة و Istanbul Finance Center.'],
      ['استراتيجية حسب الملف، لا وعد تلقائي', 'يجب على المستثمر الأجنبي فحص الإقامة الضريبية، مصدر الدخل، السنوات الثلاث السابقة، الاتفاقيات الضريبية، البنك، الوجود الحقيقي وبلد الأصل.'],
      ['حالات عملية للمستثمرين', 'الإقامة الخاصة، family office، الهولدينغ التشغيلي، المركز الإقليمي، التجارة العابرة، الصناعة التصديرية أو قاعدة Istanbul Finance Center يمكن دراستها كل على حدة.'],
      ['ملف يجب توثيقه', 'النهج الصحيح هو تحضير إثبات الإقامة، مصدر الأموال، بنية الدخل، الشركات، الأصول، العائلة، الميراث وجدول الانتقال.'],
    ],
  };
  return data[locale];
}
...
function topicBullets(key: TaxClusterKey, locale: Locale): Array<[string,string]> {
  ...
  const fr: Record<TaxClusterKey, Array<[string,string]>> = {
    ...
    twentyYearExemption: [['Revenus étrangers','L’objectif est de distinguer revenus étrangers et revenus de source turque.'],['Condition des 3 ans','Le profil doit vérifier l’absence de résidence ou assujettissement fiscal turc récent.'],['Patrimoine','Le taux de 1 % sur certaines transmissions rend le sujet familial et patrimonial.'],['Diagnostic','Il faut cartographier pays de résidence, revenus, actifs, conventions fiscales et preuves.']],
