...
function formClass(extra = '') {
  return `relative border border-[#d8c7a1] bg-[#fffaf0] p-6 shadow-[0_35px_100px_rgba(16,24,39,0.12)] md:p-8 ${extra}`;
}

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="block"><span className="mb-2 block text-xs font-bold uppercase tracking-[0.16em] text-[#5c6676]">{label}</span>{children}</label>;
}

function SelectField({ name, options }: { name: string; options: string[] }) {
  return <select name={name} className={inputClass()} defaultValue=""><option value="">—</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select>;
}
