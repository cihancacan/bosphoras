...
type CopyBase = {
  eyebrow: string; title: string; submit: string; sending: string; success: string; error: string;
};

type DiagnosticCopy = CopyBase & {
  fullName: string; phone: string; email: string; residence: string; city: string; subject: string; timeline: string; budget: string; message: string; consent: string;
  placeholders: { name: string; phone: string; email: string; residence: string; message: string };
  cityOptions: string[]; subjectOptions: string[]; timelineOptions: string[]; budgetOptions: string[];
};

type MembershipCopy = CopyBase & {
  fullName: string; whatsapp: string; email: string; residence: string; referrer: string; profile: string; company: string; website: string; annualBudget: string; mainNeed: string; motivation: string;
  accessTitle: string; accessText: string; annualAdmission: string; monthlyRetainer: string; consentInfo: string; consentValidation: string;
  ...
  profileOptions: string[]; budgetOptions: string[]; serviceOptions: string[];
};

const statusMessages = {
  fr: { sending: 'Envoi en cours…', success: 'Votre demande a bien été envoyée. Un email de confirmation vient de vous être adressé.', error: 'L’envoi n’a pas abouti. Vous pouvez réessayer ou écrire directement à contact@bosphoras.com.' },
  en: { sending: 'Sending…', success: 'Your request has been sent. A confirmation email has just been sent to you.', error: 'The request could not be sent. You may try again or write directly to contact@bosphoras.com.' },
  ru: { sending: 'Отправка…', success: 'Ваш запрос отправлен. Подтверждение было отправлено вам по email.', error: 'Отправка не удалась. Вы можете повторить попытку или написать напрямую на contact@bosphoras.com.' },
  ar: { sending: 'جارٍ الإرسال…', success: 'تم إرسال طلبكم. تم إرسال رسالة تأكيد إلى بريدكم الإلكتروني.', error: 'تعذر إرسال الطلب. يمكنكم المحاولة مرة أخرى أو الكتابة مباشرة إلى contact@bosphoras.com.' },
};

const diagnostic: Record<Locale, DiagnosticCopy> = {
  fr: { eyebrow: 'Formulaire confidentiel', title: 'Votre demande', fullName: 'Nom complet *', phone: 'Téléphone *', email: 'Email *', residence: 'Pays de résidence', city: 'Ville ciblée', subject: 'Sujet principal *', timeline: 'Délai souhaité', budget: 'Budget / enveloppe', message: 'Message *', consent: 'J’accepte que Bosphoras me contacte au sujet de cette demande. Je comprends que les informations transmises seront traitées avec confidentialité.', submit: 'Envoyer ma demande', ...statusMessages.fr, placeholders: { name: 'Votre nom', phone: '+33...', email: 'votre@email.com', residence: 'France, UAE, Kazakhstan...', message: 'Expliquez votre situation, vos objectifs et ce que vous souhaitez organiser en Turquie.' }, cityOptions: ['Istanbul', 'Bodrum', 'Antalya', 'Plusieurs villes', 'Je ne sais pas encore'], subjectOptions: ['Installation en Turquie', 'Fiscalité / résidence fiscale', 'Création d’entreprise', 'Immobilier / relocation', 'Santé / assurance', 'Transport VIP / hospitality', 'Accès privé / membership', 'Autre demande'], timelineOptions: ['Urgent', 'Sous 30 jours', '1 à 3 mois', '3 à 6 mois', 'Projet exploratoire'], budgetOptions: ['À définir', '50k - 150k', '150k - 500k', '500k - 1M', '1M+'] },
  en: { eyebrow: 'Confidential form', title: 'Your request', fullName: 'Full name *', phone: 'Phone *', email: 'Email *', residence: 'Country of residence', city: 'Target city', subject: 'Main subject *', timeline: 'Desired timeline', budget: 'Budget / envelope', message: 'Message *', consent: 'I agree that Bosphoras may contact me regarding this request. I understand that the information submitted will be treated confidentially.', submit: 'Send my request', ...statusMessages.en, placeholders: { name: 'Your name', phone: '+44...', email: 'your@email.com', residence: 'United Kingdom, UAE, Kazakhstan...', message: 'Explain your situation, your objectives and what you would like to organize in Turkey.' }, cityOptions: ['Istanbul', 'Bodrum', 'Antalya', 'Several cities', 'I do not know yet'], subjectOptions: ['Relocation to Turkey', 'Tax / tax residency', 'Company formation', 'Property / relocation', 'Health / insurance', 'VIP transport / hospitality', 'Private access / membership', 'Other request'], timelineOptions: ['Urgent', 'Within 30 days', '1 to 3 months', '3 to 6 months', 'Exploratory project'], budgetOptions: ['To be defined', '50k - 150k', '150k - 500k', '500k - 1M', '1M+'] },
  ru: { eyebrow: 'Конфиденциальная форма', title: 'Ваш запрос', fullName: 'Полное имя *', phone: 'Телефон *', email: 'Email *', residence: 'Страна проживания', city: 'Целевой город', subject: 'Основная тема *', timeline: 'Желаемые сроки', budget: 'Бюджет / ориентировочная сумма', message: 'Сообщение *', consent: 'Я согласен/согласна, чтобы Bosphoras связался со мной по этому запросу. Я понимаю, что переданная информация будет обработана конфиденциально.', submit: 'Отправить запрос', ...statusMessages.ru, placeholders: { name: 'Ваше имя', phone: '+7...', email: 'your@email.com', residence: 'Казахстан, ОАЭ, Франция...', message: 'Опишите вашу ситуацию, цели и то, что вы хотите организовать в Турции.' }, cityOptions: ['Стамбул', 'Бодрум', 'Анталья', 'Несколько городов', 'Я пока не знаю'], subjectOptions: ['Переезд в Турцию', 'Налоги / налоговая резиденция', 'Создание компании', 'Недвижимость / relocation', 'Здоровье / страхование', 'VIP transport / hospitality', 'Private access / membership', 'Другой запрос'], timelineOptions: ['Срочно', 'В течение 30 дней', '1–3 месяца', '3–6 месяцев', 'Ознакомительный проект'], budgetOptions: ['Определить позже', '50k - 150k', '150k - 500k', '500k - 1M', '1M+'] },
  ar: { eyebrow: 'نموذج سري', title: 'طلبكم', fullName: 'الاسم الكامل *', phone: 'الهاتف *', email: 'البريد الإلكتروني *', residence: 'بلد الإقامة', city: 'المدينة المستهدفة', subject: 'الموضوع الرئيسي *', timeline: 'المهلة المطلوبة', budget: 'الميزانية / الإطار المالي', message: 'الرسالة *', consent: 'أوافق على أن يتواصل معي Bosphoras بخصوص هذا الطلب. أفهم أن المعلومات المرسلة ستتم معالجتها بسرية.', submit: 'إرسال الطلب', ...statusMessages.ar, placeholders: { name: 'اسمكم', phone: '+971...', email: 'your@email.com', residence: 'الإمارات، السعودية، كازاخستان...', message: 'اشرحوا وضعكم وأهدافكم وما ترغبون في تنظيمه في تركيا.' }, cityOptions: ['إسطنبول', 'بودروم', 'أنطاليا', 'عدة مدن', 'لا أعرف بعد'], subjectOptions: ['الانتقال إلى تركيا', 'الضرائب / الإقامة الضريبية', 'تأسيس شركة', 'العقار / relocation', 'الصحة / التأمين', 'النقل VIP / hospitality', 'الوصول الخاص / العضوية', 'طلب آخر'], timelineOptions: ['عاجل', 'خلال 30 يوماً', '1 إلى 3 أشهر', '3 إلى 6 أشهر', 'مشروع استكشافي'], budgetOptions: ['يحدد لاحقاً', '50k - 150k', '150k - 500k', '500k - 1M', '1M+'] },
};

const membershipBase = {
  fr: { eyebrow: 'Candidature membre', title: 'Votre profil', fullName: 'Nom complet *', whatsapp: 'Téléphone WhatsApp *', email: 'Email *', residence: 'Pays de résidence *', referrer: 'Parrain / recommandation', profile: 'Profil principal *', company: 'Société / activité', website: 'LinkedIn / site internet', annualBudget: 'Budget annuel estimé', mainNeed: 'Besoin principal *', motivation: 'Pourquoi souhaitez-vous rejoindre Bosphoras Private Access ? *', accessTitle: 'Conditions d’accès', accessText: 'Private Access fonctionne sur un cycle de 12 mois à compter de la validation du profil. Les conditions sont révisables chaque année.', consentInfo: 'Je confirme que les informations transmises sont exactes et j’accepte que Bosphoras me contacte concernant ma demande d’adhésion.', consentValidation: 'Je comprends que l’adhésion est soumise à validation et peut être refusée sans obligation de justification.', submit: 'Envoyer ma candidature', ...statusMessages.fr, placeholders: { name: 'Votre nom', phone: '+33...', email: 'votre@email.com', residence: 'France, UAE, Kazakhstan...', referrer: 'Nom du parrain, si applicable', company: 'Nom de société, secteur, activité', website: 'Lien LinkedIn ou site', motivation: 'Expliquez brièvement votre situation, votre projet en Turquie et ce que vous attendez du bureau privé Bosphoras.' }, profileOptions: ['Entrepreneur', 'Investisseur', 'Dirigeant', 'Profession libérale', 'Family office', 'Autre'], budgetOptions: ['Moins de 25 000 USD', '25 000 - 50 000 USD', '50 000 - 100 000 USD', '100 000 USD+', 'Préfère en discuter en privé'], serviceOptions: ['Installation en Turquie', 'Business / société / fiscalité', 'Santé / assurance', 'Immobilier / relocation', 'Transport VIP / hospitality', 'Accès privé / lifestyle', 'Plusieurs besoins'] },
  en: { eyebrow: 'Member application', title: 'Your profile', fullName: 'Full name *', whatsapp: 'WhatsApp phone *', email: 'Email *', residence: 'Country of residence *', referrer: 'Referrer / recommendation', profile: 'Main profile *', company: 'Company / activity', website: 'LinkedIn / website', annualBudget: 'Estimated annual budget', mainNeed: 'Main need *', motivation: 'Why do you want to join Bosphoras Private Access? *', accessTitle: 'Access conditions', accessText: 'Private Access operates on a 12-month cycle from profile validation. Conditions may be reviewed each year.', consentInfo: 'I confirm that the information submitted is accurate and I agree that Bosphoras may contact me regarding my membership application.', consentValidation: 'I understand that membership is subject to validation and may be refused without any obligation to provide justification.', submit: 'Send my application', ...statusMessages.en, placeholders: { name: 'Your name', phone: '+44...', email: 'your@email.com', residence: 'United Kingdom, UAE, Kazakhstan...', referrer: 'Referrer name, if applicable', company: 'Company name, sector, activity', website: 'LinkedIn or website link', motivation: 'Briefly explain your situation, your project in Turkey and what you expect from the Bosphoras private office.' }, profileOptions: ['Entrepreneur', 'Investor', 'Executive', 'Liberal profession', 'Family office', 'Other'], budgetOptions: ['Less than 25,000 USD', '25,000 - 50,000 USD', '50,000 - 100,000 USD', '100,000 USD+', 'Prefer to discuss privately'], serviceOptions: ['Relocation to Turkey', 'Business / company / tax', 'Health / insurance', 'Property / relocation', 'VIP transport / hospitality', 'Private access / lifestyle', 'Several needs'] },
  ru: { eyebrow: 'Заявка участника', title: 'Ваш профиль', fullName: 'Полное имя *', whatsapp: 'Телефон WhatsApp *', email: 'Email *', residence: 'Страна проживания *', referrer: 'Рекомендатель / рекомендация', profile: 'Основной профиль *', company: 'Компания / деятельность', website: 'LinkedIn / сайт', annualBudget: 'Ориентировочный годовой бюджет', mainNeed: 'Главная потребность *', motivation: 'Почему вы хотите присоединиться к Bosphoras Private Access? *', accessTitle: 'Условия доступа', accessText: 'Private Access работает по циклу 12 месяцев с момента утверждения профиля. Условия могут пересматриваться ежегодно.', consentInfo: 'Я подтверждаю, что переданная информация точна, и соглашаюсь, чтобы Bosphoras связался со мной по моей заявке на членство.', consentValidation: 'Я понимаю, что членство подлежит утверждению и может быть отказано без обязанности объяснять причины.', submit: 'Отправить заявку', ...statusMessages.ru, placeholders: { name: 'Ваше имя', phone: '+7...', email: 'your@email.com', residence: 'Казахстан, ОАЭ, Франция...', referrer: 'Имя рекомендателя, если применимо', company: 'Название компании, сектор, деятельность', website: 'Ссылка LinkedIn или сайт', motivation: 'Кратко опишите вашу ситуацию, проект в Турции и ожидания от частного офиса Bosphoras.' }, profileOptions: ['Предприниматель', 'Инвестор', 'Руководитель', 'Свободная профессия', 'Family office', 'Другое'], budgetOptions: ['Менее 25 000 USD', '25 000 - 50 000 USD', '50 000 - 100 000 USD', '100 000 USD+', 'Предпочитаю обсудить лично'], serviceOptions: ['Переезд в Турцию', 'Бизнес / компания / налоги', 'Здоровье / страхование', 'Недвижимость / relocation', 'VIP transport / hospitality', 'Private access / lifestyle', 'Несколько потребностей'] },
  ar: { eyebrow: 'طلب عضوية', title: 'ملفكم', fullName: 'الاسم الكامل *', whatsapp: 'هاتف WhatsApp *', email: 'البريد الإلكتروني *', residence: 'بلد الإقامة *', referrer: 'المُرشح / التوصية', profile: 'الملف الرئيسي *', company: 'الشركة / النشاط', website: 'LinkedIn / الموقع الإلكتروني', annualBudget: 'الميزانية السنوية التقديرية', mainNeed: 'الحاجة الرئيسية *', motivation: 'لماذا ترغبون في الانضمام إلى Bosphoras Private Access؟ *', accessTitle: 'شروط الوصول', accessText: 'يعمل Private Access على دورة مدتها 12 شهراً من تاريخ اعتماد الملف. يمكن مراجعة الشروط كل سنة.', consentInfo: 'أؤكد أن المعلومات المرسلة صحيحة وأوافق على أن يتواصل معي Bosphoras بخصوص طلب العضوية.', consentValidation: 'أفهم أن العضوية خاضعة للموافقة ويمكن رفضها دون التزام بتقديم مبرر.', submit: 'إرسال الطلب', ...statusMessages.ar, placeholders: { name: 'اسمكم', phone: '+971...', email: 'your@email.com', residence: 'الإمارات، السعودية، كازاخستان...', referrer: 'اسم المُرشح، إن وجد', company: 'اسم الشركة، القطاع، النشاط', website: 'رابط LinkedIn أو الموقع', motivation: 'اشرحوا بإيجاز وضعكم ومشروعكم في تركيا وما تتوقعونه من المكتب الخاص Bosphoras.' }, profileOptions: ['رائد أعمال', 'مستثمر', 'مدير', 'مهنة حرة', 'Family office', 'آخر'], budgetOptions: ['أقل من 25,000 USD', '25,000 - 50,000 USD', '50,000 - 100,000 USD', '100,000 USD+', 'أفضل المناقشة بسرية'], serviceOptions: ['الانتقال إلى تركيا', 'الأعمال / الشركة / الضرائب', 'الصحة / التأمين', 'العقار / relocation', 'النقل VIP / hospitality', 'الوصول الخاص / lifestyle', 'عدة احتياجات'] },
};

function getMembershipCopy(locale: Locale): MembershipCopy {
  return {
    ...membershipBase[locale],
    annualAdmission: annualAdmissionLine(locale),
    monthlyRetainer: monthlyRetainerLine(locale),
  };
}
...
async function submitForm(event: React.FormEvent<HTMLFormElement>, locale: Locale, formKind: FormKind, setStatus: (status: Status) => void) {
...
export function BosphorasMembershipForm({ locale, embedded = false }: { locale: Locale; embedded?: boolean }) {
  ...
  const [status, setStatus] = useState<Status>('idle');
  const form = (
    <form onSubmit={(event) => submitForm(event, locale, 'membership-application', setStatus)} className={formClass(embedded ? '' : 'mx-auto max-w-5xl')}>
      <div className="mb-8 border-b border-[#d8c7a1] pb-6"><p className="text-[0.62rem] font-bold uppercase tracking-[0.26em] text-[#8a6728]">{copy.eyebrow}</p><h2 className="mt-3 font-serif text-3xl tracking-[-0.03em] text-[#121826] md:text-4xl">{copy.title}</h2></div>
      <div className="grid gap-4 md:grid-cols-2">
        <Field label={copy.fullName}><input required name={copy.fullName.replace(' *', '')} type="text" placeholder={copy.placeholders.name} className={inputClass()} /></Field>
      ...
      <div className="mt-6 border border-[#d8c7a1] bg-[#f8f1e7] p-5"><p className="font-serif text-2xl text-[#121826]">{copy.accessTitle}</p><p className="mt-3 text-sm leading-7 text-[#3e4857]">{priceText(copy.accessText)}</p><div className="mt-4 grid gap-3 text-sm leading-7 text-[#3e4857] sm:grid-cols-2"><p><strong>{copy.annualAdmission.split(':')[0]} :</strong> {copy.annualAdmission.split(':').slice(1).join(':').trim()}</p><p><strong>{copy.monthlyRetainer.split(':')[0]} :</strong> {copy.monthlyRetainer.split(':').slice(1).join(':').trim()}</p></div></div>
